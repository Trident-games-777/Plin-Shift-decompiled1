...
public final class zzgch {
    ...
    public static int zza(double d, RoundingMode roundingMode) {
        boolean z;
        boolean z2 = false;
        zzfwq.zzf(d > 0.0d && zzgci.zzb(d), "x must be positive and finite");
    ...
    public static boolean zzc(double d) {
        if (zzgci.zzb(d)) {
            return d == 0.0d || 52 - Long.numberOfTrailingZeros(zzgci.zza(d)) <= Math.getExponent(d);
        }
        return false;
    }

    public static boolean zzd(double d) {
        if (d > 0.0d && zzgci.zzb(d)) {
            long zza = zzgci.zza(d);
            if ((zza & (-1 + zza)) == 0) {
                return true;
            }
        }

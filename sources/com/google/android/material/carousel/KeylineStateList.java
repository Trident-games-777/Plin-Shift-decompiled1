...
class KeylineStateList {
    ...
    private static KeylineState shiftKeylineStateForPadding(KeylineState keylineState, float f, float f2, boolean z, float f3) {
        ...
        while (i < arrayList.size()) {
            ...
            if (keyline.isAnchor) {
                ...
                float childMaskPercentage = CarouselStrategy.getChildMaskPercentage(f6, keylineState.getItemSize(), f3);
                float f7 = (f6 / 2.0f) + f4;
    ...
    private static List<KeylineState> getStateStepsStart(Carousel carousel, KeylineState keylineState, float f, float f2) {
        ...
        if (!isFirstFocalItemAtLeftOfContainer(keylineState2) && findFirstNonAnchorKeylineIndex != -1) {
            ...
            float f5 = keylineState2.getFirstKeyline().locOffset - (keylineState2.getFirstKeyline().maskedItemSize / 2.0f);
            if (firstFocalKeylineIndex > 0 || keylineState2.getFirstFocalKeyline().cutoff <= 0.0f) {
                ...
                while (i < firstFocalKeylineIndex) {
                    ...
                    float f7 = f6 + keylineState2.getKeylines().get(i2).cutoff;
                    int i3 = i2 - 1;
                    if (i3 >= 0) {
                        size = findFirstIndexAfterLastFocalKeylineWithMask(keylineState3, keylineState2.getKeylines().get(i3).mask) - 1;
                    }
                    KeylineState moveKeylineAndCreateKeylineState = moveKeylineAndCreateKeylineState(keylineState3, findFirstNonAnchorKeylineIndex, size, f5 + f7, (keylineState2.getFirstFocalKeylineIndex() - i) - 1, (keylineState2.getLastFocalKeylineIndex() - i) - 1, containerWidth);
                    if (i == firstFocalKeylineIndex - 1 && f4 > 0.0f) {
                ...
                arrayList.add(shiftKeylinesAndCreateKeylineState(keylineState2, f5 + keylineState2.getFirstFocalKeyline().cutoff, containerWidth));
                return arrayList;
            }
        } else if (f4 > 0.0f) {
        ...
        if (!isLastFocalItemVisibleAtRightOfContainer(carousel, keylineState) && findLastNonAnchorKeylineIndex != -1) {
            ...
            if (lastFocalKeylineIndex > 0 || keylineState2.getLastFocalKeyline().cutoff <= 0.0f) {
                ...
                while (i < lastFocalKeylineIndex) {
                    ...
                    float f7 = f6 + keylineState2.getKeylines().get(i2).cutoff;
                    int i3 = i2 + 1;
                    KeylineState moveKeylineAndCreateKeylineState = moveKeylineAndCreateKeylineState(keylineState3, findLastNonAnchorKeylineIndex, i3 < keylineState2.getKeylines().size() ? findLastIndexBeforeFirstFocalKeylineWithMask(keylineState3, keylineState2.getKeylines().get(i3).mask) + 1 : 0, f5 - f7, keylineState2.getFirstFocalKeylineIndex() + i + 1, keylineState2.getLastFocalKeylineIndex() + i + 1, containerWidth);
                    if (i == lastFocalKeylineIndex - 1 && f4 > 0.0f) {
                ...
                arrayList.add(shiftKeylinesAndCreateKeylineState(keylineState2, f5 - keylineState2.getLastFocalKeyline().cutoff, containerWidth));
                return arrayList;
            }
        } else if (f4 > 0.0f) {

...
public final class UncontainedCarouselStrategy extends CarouselStrategy {
    ...
    public KeylineState onFirstChildMeasuredWithMargins(Carousel carousel, View view) {
        ...
        if (carousel.getCarouselAlignment() == 1) {
            float f6 = f5 / 2.0f;
            KeylineState createCenterAlignedKeylineState = createCenterAlignedKeylineState(containerWidth, f3, f4, max, Math.max(Math.min(3.0f * f6, f4), getSmallItemSizeMin() + f3), extraSmallSize2, f6);
            return createCenterAlignedKeylineState;
        }
        int i = 1;
        float f7 = f4;
        float f8 = containerWidth;
        if (f5 <= 0.0f) {
            i = 0;
        }
        return createLeftAlignedKeylineState(view.getContext(), f3, f8, f7, max, calculateMediumChildSize(extraSmallSize, f7, f5), i, extraSmallSize2);
    }
    ...
    private KeylineState createLeftAlignedKeylineState(Context context, float f, float f2, float f3, int i, float f4, int i2, float f5) {
        ...
        int i3 = i;
        float f9 = (((float) i3) * f7) + 0.0f;
        KeylineState.Builder addKeylineRange = new KeylineState.Builder(f7, f2).addAnchorKeyline(0.0f - (max / 2.0f), childMaskPercentage, max).addKeylineRange(f7 / 2.0f, 0.0f, f7, i3, true);
        if (i2 > 0) {

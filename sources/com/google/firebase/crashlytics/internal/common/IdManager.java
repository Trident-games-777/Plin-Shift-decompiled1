...
public class IdManager implements InstallIdProvider {
    ...
    private boolean shouldRefresh() {
        ...
        if (installIds2 != null) {
            return installIds2.getFirebaseInstallationId() == null && this.dataCollectionArbiter.isAutomaticDataCollectionEnabled();
        }
        return true;
    }
    ...
    public FirebaseInstallationId fetchTrueFid(boolean z) {
        ...
        if (z) {
            try {
                str = ((InstallationTokenResult) Tasks.await(this.firebaseInstallations.getToken(false), WorkRequest.MIN_BACKOFF_MILLIS, TimeUnit.MILLISECONDS)).getToken();
            } catch (Exception e) {
                Logger.getLogger().w("Error getting Firebase authentication token.", e);
            }
            str2 = (String) Tasks.await(this.firebaseInstallations.getId(), WorkRequest.MIN_BACKOFF_MILLIS, TimeUnit.MILLISECONDS);

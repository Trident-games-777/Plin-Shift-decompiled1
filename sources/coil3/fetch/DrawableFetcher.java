...
public final class DrawableFetcher implements Fetcher {
    ...
    public Object fetch(Continuation<? super FetchResult> continuation) {
        ...
        if (isVector) {
            drawable = new BitmapDrawable(this.options.getContext().getResources(), DrawableUtils.INSTANCE.convertToBitmap(this.data, ImageRequests_androidKt.getBitmapConfig(this.options), this.options.getSize(), this.options.getScale(), this.options.getPrecision() == Precision.INEXACT));
        } else {
            drawable = this.data;
        }

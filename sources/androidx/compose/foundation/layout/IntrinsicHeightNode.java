...
final class IntrinsicHeightNode extends IntrinsicSizeModifier {
    ...
    public long m724calculateContentConstraintsl58MMJ0(MeasureScope measureScope, Measurable measurable, long j) {
        int i;
        if (this.height == IntrinsicSize.Min) {
            i = measurable.minIntrinsicHeight(Constraints.m7052getMaxWidthimpl(j));
        } else {
            i = measurable.maxIntrinsicHeight(Constraints.m7052getMaxWidthimpl(j));
        }
        if (i < 0) {
            i = 0;
        }
        return Constraints.Companion.m7063fixedHeightOenEA2s(i);
    }

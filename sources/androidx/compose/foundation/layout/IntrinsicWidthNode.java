...
final class IntrinsicWidthNode extends IntrinsicSizeModifier {
    ...
    public long m727calculateContentConstraintsl58MMJ0(MeasureScope measureScope, Measurable measurable, long j) {
        int i;
        if (this.width == IntrinsicSize.Min) {
            i = measurable.minIntrinsicWidth(Constraints.m7051getMaxHeightimpl(j));
        } else {
            i = measurable.maxIntrinsicWidth(Constraints.m7051getMaxHeightimpl(j));
        }
        if (i < 0) {
            i = 0;
        }
        return Constraints.Companion.m7064fixedWidthOenEA2s(i);
    }

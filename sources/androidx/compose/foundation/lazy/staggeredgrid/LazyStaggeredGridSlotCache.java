...
final class LazyStaggeredGridSlotCache implements LazyGridStaggeredGridSlotsProvider {
    ...
    public LazyStaggeredGridSlots m1070invoke0kLqBqw(Density density, long j) {
        if (this.cachedSizes == null || !Constraints.m7045equalsimpl0(this.cachedConstraints, j) || this.cachedDensity != density.getDensity()) {
            this.cachedConstraints = j;
            this.cachedDensity = density.getDensity();
            LazyStaggeredGridSlots invoke = this.calculation.invoke(density, Constraints.m7040boximpl(j));

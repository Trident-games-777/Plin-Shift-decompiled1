    ...
    public static final TransferableContent consume(TransferableContent transferableContent, Function1<? super ClipData.Item, Boolean> function1) {
        ...
        if (clipData.getItemCount() != 1) {
            ...
            if (list.size() != clipData.getItemCount()) {
                ...
                ClipData clipData2 = new ClipData(clipDescription, (ClipData.Item) CollectionsKt.first(list));
                int size = list.size();
                for (int i2 = 1; i2 < size; i2++) {
                    clipData2.addItem((ClipData.Item) list.get(i2));
                }
                return new TransferableContent(AndroidClipboardManager_androidKt.toClipEntry(clipData2), AndroidClipboardManager_androidKt.toClipMetadata(clipDescription), transferableContent.m357getSourcekB6V9T0(), transferableContent.getPlatformTransferableContent(), (DefaultConstructorMarker) null);
            }

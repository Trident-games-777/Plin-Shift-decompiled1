...
public final class AndroidRippleIndicationInstance extends RippleIndicationInstance implements RememberObserver, RippleHostKey {
    ...
    public void drawIndication(ContentDrawScope contentDrawScope) {
        int i;
        this.rippleSize = contentDrawScope.m4865getSizeNHjbRc();
        if (Float.isNaN(this.radius)) {
            i = MathKt.roundToInt(RippleAnimationKt.m1684getRippleEndRadiuscSwnlzA(contentDrawScope, this.bounded, contentDrawScope.m4865getSizeNHjbRc()));
        } else {
            i = contentDrawScope.m7086roundToPx0680j_4(this.radius);
        }
        this.rippleRadius = i;
        long r4 = this.color.getValue().m4249unboximpl();
        float pressedAlpha = this.rippleAlpha.getValue().getPressedAlpha();
        contentDrawScope.drawContent();
        DrawScope drawScope = contentDrawScope;
        m1689drawStateLayerH2RKhps(drawScope, this.radius, r4);
        Canvas canvas = drawScope.getDrawContext().getCanvas();
        getInvalidateTick();
        RippleHostView rippleHostView = getRippleHostView();
        if (rippleHostView != null) {
            rippleHostView.m1688setRippleProperties07v42R4(contentDrawScope.m4865getSizeNHjbRc(), r4, pressedAlpha);
    ...
    public void addRipple(PressInteraction.Press press, CoroutineScope coroutineScope) {
        ...
        rippleHostView.m1687addRippleKOepWvA(press, this.bounded, this.rippleSize, this.rippleRadius, this.color.getValue().m4249unboximpl(), this.rippleAlpha.getValue().getPressedAlpha(), this.onInvalidateRipple);
    ...
    private final RippleContainer getOrCreateRippleContainer() {
        ...
        RippleContainer access$createAndAttachRippleContainerIfNeeded = Ripple_androidKt.createAndAttachRippleContainerIfNeeded(this.view);

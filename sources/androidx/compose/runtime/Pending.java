package androidx.compose.runtime;

import androidx.collection.IntObjectMap;
import androidx.collection.MutableIntObjectMap;
import androidx.collection.MutableScatterMap;
import java.util.ArrayList;
import java.util.List;
import kotlin.Lazy;
import kotlin.LazyKt;
import kotlin.Metadata;
import kotlin.jvm.internal.DefaultConstructorMarker;
import kotlin.jvm.internal.Intrinsics;

@Metadata(d1 = {"\u0000F\n\u0002\u0018\u0002\n\u0002\u0010\u0000\n\u0000\n\u0002\u0010!\n\u0002\u0018\u0002\n\u0000\n\u0002\u0010\b\n\u0002\b\u0007\n\u0002\u0018\u0002\n\u0002\u0018\u0002\n\u0002\b\u0003\n\u0002\u0018\u0002\n\u0002\b\u0006\n\u0002\u0010 \n\u0002\b\b\n\u0002\u0010\u000b\n\u0000\n\u0002\u0010\u0002\n\u0002\b\f\b\u0002\u0018\u00002\u00020\u0001B\u001b\u0012\f\u0010\u0002\u001a\b\u0012\u0004\u0012\u00020\u00040\u0003\u0012\u0006\u0010\u0005\u001a\u00020\u0006¢\u0006\u0002\u0010\u0007J\u001a\u0010\u001d\u001a\u0004\u0018\u00010\u00042\u0006\u0010\u001e\u001a\u00020\u00062\b\u0010\u001f\u001a\u0004\u0018\u00010\u0001J\u000e\u0010 \u001a\u00020\u00062\u0006\u0010!\u001a\u00020\u0004J\u000e\u0010\"\u001a\u00020#2\u0006\u0010!\u001a\u00020\u0004J\u0016\u0010$\u001a\u00020%2\u0006\u0010!\u001a\u00020\u00042\u0006\u0010&\u001a\u00020\u0006J\u001e\u0010'\u001a\u00020%2\u0006\u0010(\u001a\u00020\u00062\u0006\u0010)\u001a\u00020\u00062\u0006\u0010*\u001a\u00020\u0006J\u0016\u0010+\u001a\u00020%2\u0006\u0010(\u001a\u00020\u00062\u0006\u0010)\u001a\u00020\u0006J\u000e\u0010,\u001a\u00020\u00062\u0006\u0010!\u001a\u00020\u0004J\u0016\u0010-\u001a\u00020#2\u0006\u0010.\u001a\u00020\u00062\u0006\u0010/\u001a\u00020\u0006J\u000e\u00100\u001a\u00020\u00062\u0006\u0010!\u001a\u00020\u0004R\u001a\u0010\b\u001a\u00020\u0006X\u000e¢\u0006\u000e\n\u0000\u001a\u0004\b\t\u0010\n\"\u0004\b\u000b\u0010\fR\u0014\u0010\r\u001a\b\u0012\u0004\u0012\u00020\u000f0\u000eX\u0004¢\u0006\u0002\n\u0000R\u0017\u0010\u0002\u001a\b\u0012\u0004\u0012\u00020\u00040\u0003¢\u0006\b\n\u0000\u001a\u0004\b\u0010\u0010\u0011R-\u0010\u0012\u001a\u000e\u0012\u0004\u0012\u00020\u0001\u0012\u0004\u0012\u00020\u00040\u00138FX\u0002ø\u0001\u0000ø\u0001\u0001¢\u0006\f\n\u0004\b\u0016\u0010\u0017\u001a\u0004\b\u0014\u0010\u0015R\u0011\u0010\u0005\u001a\u00020\u0006¢\u0006\b\n\u0000\u001a\u0004\b\u0018\u0010\nR\u0017\u0010\u0019\u001a\b\u0012\u0004\u0012\u00020\u00040\u001a8F¢\u0006\u0006\u001a\u0004\b\u001b\u0010\u0011R\u0014\u0010\u001c\u001a\b\u0012\u0004\u0012\u00020\u00040\u0003X\u0004¢\u0006\u0002\n\u0000\u0002\u000b\n\u0005\b¡\u001e0\u0001\n\u0002\b!¨\u00061"}, d2 = {"Landroidx/compose/runtime/Pending;", "", "keyInfos", "", "Landroidx/compose/runtime/KeyInfo;", "startIndex", "", "(Ljava/util/List;I)V", "groupIndex", "getGroupIndex", "()I", "setGroupIndex", "(I)V", "groupInfos", "Landroidx/collection/MutableIntObjectMap;", "Landroidx/compose/runtime/GroupInfo;", "getKeyInfos", "()Ljava/util/List;", "keyMap", "Landroidx/compose/runtime/MutableScatterMultiMap;", "getKeyMap-SAeQiB4", "()Landroidx/collection/MutableScatterMap;", "keyMap$delegate", "Lkotlin/Lazy;", "getStartIndex", "used", "", "getUsed", "usedKeys", "getNext", "key", "dataKey", "nodePositionOf", "keyInfo", "recordUsed", "", "registerInsert", "", "insertIndex", "registerMoveNode", "from", "to", "count", "registerMoveSlot", "slotPositionOf", "updateNodeCount", "group", "newCount", "updatedNodeCountOf", "runtime_release"}, k = 1, mv = {1, 8, 0}, xi = 48)
/* compiled from: Composer.kt */
final class Pending {
    private int groupIndex;
    private final MutableIntObjectMap<GroupInfo> groupInfos;
    private final List<KeyInfo> keyInfos;
    private final Lazy keyMap$delegate;
    private final int startIndex;
    private final List<KeyInfo> usedKeys;

    public Pending(List<KeyInfo> list, int i) {
        this.keyInfos = list;
        this.startIndex = i;
        if (!(i >= 0)) {
            PreconditionsKt.throwIllegalArgumentException("Invalid start index");
        }
        this.usedKeys = new ArrayList();
        Pending pending = this;
        MutableIntObjectMap<GroupInfo> mutableIntObjectMap = new MutableIntObjectMap<>(0, 1, (DefaultConstructorMarker) null);
        int size = this.keyInfos.size();
        int i2 = 0;
        for (int i3 = 0; i3 < size; i3++) {
            KeyInfo keyInfo = this.keyInfos.get(i3);
            mutableIntObjectMap.set(keyInfo.getLocation(), new GroupInfo(i3, i2, keyInfo.getNodes()));
            i2 += keyInfo.getNodes();
        }
        this.groupInfos = mutableIntObjectMap;
        this.keyMap$delegate = LazyKt.lazy(new Pending$keyMap$2(this));
    }

    public final List<KeyInfo> getKeyInfos() {
        return this.keyInfos;
    }

    public final int getStartIndex() {
        return this.startIndex;
    }

    public final int getGroupIndex() {
        return this.groupIndex;
    }

    public final void setGroupIndex(int i) {
        this.groupIndex = i;
    }

    /* renamed from: getKeyMap-SAeQiB4  reason: not valid java name */
    public final MutableScatterMap<K, Object> m3664getKeyMapSAeQiB4() {
        return ((MutableScatterMultiMap) this.keyMap$delegate.getValue()).m3663unboximpl();
    }

    public final KeyInfo getNext(int i, Object obj) {
        return (KeyInfo) MutableScatterMultiMap.m3660popimpl(m3664getKeyMapSAeQiB4(), obj != null ? new JoinedKey(Integer.valueOf(i), obj) : Integer.valueOf(i));
    }

    public final boolean recordUsed(KeyInfo keyInfo) {
        return this.usedKeys.add(keyInfo);
    }

    public final List<KeyInfo> getUsed() {
        return this.usedKeys;
    }

    public final void registerMoveSlot(int i, int i2) {
        long j;
        char c;
        long j2;
        char c2;
        int i3 = i;
        int i4 = i2;
        char c3 = 7;
        long j3 = -9187201950435737472L;
        if (i3 > i4) {
            IntObjectMap intObjectMap = this.groupInfos;
            Object[] objArr = intObjectMap.values;
            long[] jArr = intObjectMap.metadata;
            int length = jArr.length - 2;
            if (length >= 0) {
                int i5 = 0;
                while (true) {
                    long j4 = jArr[i5];
                    if ((((~j4) << c3) & j4 & j3) != j3) {
                        int i6 = 8 - ((~(i5 - length)) >>> 31);
                        int i7 = 0;
                        while (i7 < i6) {
                            if ((j4 & 255) < 128) {
                                c2 = c3;
                                GroupInfo groupInfo = (GroupInfo) objArr[(i5 << 3) + i7];
                                j2 = j3;
                                int slotIndex = groupInfo.getSlotIndex();
                                if (slotIndex == i3) {
                                    groupInfo.setSlotIndex(i4);
                                } else if (i4 <= slotIndex && slotIndex < i3) {
                                    groupInfo.setSlotIndex(slotIndex + 1);
                                }
                            } else {
                                c2 = c3;
                                j2 = j3;
                            }
                            j4 >>= 8;
                            i7++;
                            c3 = c2;
                            j3 = j2;
                        }
                        c = c3;
                        j = j3;
                        if (i6 != 8) {
                            return;
                        }
                    } else {
                        c = c3;
                        j = j3;
                    }
                    if (i5 != length) {
                        i5++;
                        c3 = c;
                        j3 = j;
                    } else {
                        return;
                    }
                }
            }
        } else if (i4 > i3) {
            IntObjectMap intObjectMap2 = this.groupInfos;
            Object[] objArr2 = intObjectMap2.values;
            long[] jArr2 = intObjectMap2.metadata;
            int length2 = jArr2.length - 2;
            if (length2 >= 0) {
                int i8 = 0;
                while (true) {
                    long j5 = jArr2[i8];
                    if ((((~j5) << 7) & j5 & -9187201950435737472L) != -9187201950435737472L) {
                        int i9 = 8 - ((~(i8 - length2)) >>> 31);
                        for (int i10 = 0; i10 < i9; i10++) {
                            if ((j5 & 255) < 128) {
                                GroupInfo groupInfo2 = (GroupInfo) objArr2[(i8 << 3) + i10];
                                int slotIndex2 = groupInfo2.getSlotIndex();
                                if (slotIndex2 == i3) {
                                    groupInfo2.setSlotIndex(i4);
                                } else if (i3 + 1 <= slotIndex2 && slotIndex2 < i4) {
                                    groupInfo2.setSlotIndex(slotIndex2 - 1);
                                }
                            }
                            j5 >>= 8;
                        }
                        if (i9 != 8) {
                            return;
                        }
                    }
                    if (i8 != length2) {
                        i8++;
                    } else {
                        return;
                    }
                }
            }
        }
    }

    public final void registerMoveNode(int i, int i2, int i3) {
        long j;
        char c;
        long j2;
        char c2;
        int i4 = i;
        int i5 = i2;
        char c3 = 7;
        long j3 = -9187201950435737472L;
        if (i4 > i5) {
            IntObjectMap intObjectMap = this.groupInfos;
            Object[] objArr = intObjectMap.values;
            long[] jArr = intObjectMap.metadata;
            int length = jArr.length - 2;
            if (length >= 0) {
                int i6 = 0;
                while (true) {
                    long j4 = jArr[i6];
                    if ((((~j4) << c3) & j4 & j3) != j3) {
                        int i7 = 8 - ((~(i6 - length)) >>> 31);
                        int i8 = 0;
                        while (i8 < i7) {
                            if ((j4 & 255) < 128) {
                                c2 = c3;
                                GroupInfo groupInfo = (GroupInfo) objArr[(i6 << 3) + i8];
                                j2 = j3;
                                int nodeIndex = groupInfo.getNodeIndex();
                                if (i4 <= nodeIndex && nodeIndex < i4 + i3) {
                                    groupInfo.setNodeIndex((nodeIndex - i4) + i5);
                                } else if (i5 <= nodeIndex && nodeIndex < i4) {
                                    groupInfo.setNodeIndex(nodeIndex + i3);
                                }
                            } else {
                                c2 = c3;
                                j2 = j3;
                            }
                            j4 >>= 8;
                            i8++;
                            c3 = c2;
                            j3 = j2;
                        }
                        c = c3;
                        j = j3;
                        if (i7 != 8) {
                            return;
                        }
                    } else {
                        c = c3;
                        j = j3;
                    }
                    if (i6 != length) {
                        i6++;
                        c3 = c;
                        j3 = j;
                    } else {
                        return;
                    }
                }
            }
        } else if (i5 > i4) {
            IntObjectMap intObjectMap2 = this.groupInfos;
            Object[] objArr2 = intObjectMap2.values;
            long[] jArr2 = intObjectMap2.metadata;
            int length2 = jArr2.length - 2;
            if (length2 >= 0) {
                int i9 = 0;
                while (true) {
                    long j5 = jArr2[i9];
                    if ((((~j5) << 7) & j5 & -9187201950435737472L) != -9187201950435737472L) {
                        int i10 = 8 - ((~(i9 - length2)) >>> 31);
                        for (int i11 = 0; i11 < i10; i11++) {
                            if ((j5 & 255) < 128) {
                                GroupInfo groupInfo2 = (GroupInfo) objArr2[(i9 << 3) + i11];
                                int nodeIndex2 = groupInfo2.getNodeIndex();
                                if (i4 <= nodeIndex2 && nodeIndex2 < i4 + i3) {
                                    groupInfo2.setNodeIndex((nodeIndex2 - i4) + i5);
                                } else if (i4 + 1 <= nodeIndex2 && nodeIndex2 < i5) {
                                    groupInfo2.setNodeIndex(nodeIndex2 - i3);
                                }
                            }
                            j5 >>= 8;
                        }
                        if (i10 != 8) {
                            return;
                        }
                    }
                    if (i9 != length2) {
                        i9++;
                    } else {
                        return;
                    }
                }
            }
        }
    }

    public final void registerInsert(KeyInfo keyInfo, int i) {
        this.groupInfos.set(keyInfo.getLocation(), new GroupInfo(-1, i, 0));
    }

    public final boolean updateNodeCount(int i, int i2) {
        int nodeIndex;
        int i3 = i2;
        GroupInfo groupInfo = this.groupInfos.get(i);
        if (groupInfo == null) {
            return false;
        }
        int nodeIndex2 = groupInfo.getNodeIndex();
        int nodeCount = i3 - groupInfo.getNodeCount();
        groupInfo.setNodeCount(i3);
        if (nodeCount == 0) {
            return true;
        }
        IntObjectMap intObjectMap = this.groupInfos;
        Object[] objArr = intObjectMap.values;
        long[] jArr = intObjectMap.metadata;
        int length = jArr.length - 2;
        if (length < 0) {
            return true;
        }
        int i4 = 0;
        while (true) {
            long j = jArr[i4];
            if ((((~j) << 7) & j & -9187201950435737472L) != -9187201950435737472L) {
                int i5 = 8 - ((~(i4 - length)) >>> 31);
                for (int i6 = 0; i6 < i5; i6++) {
                    if ((255 & j) < 128) {
                        GroupInfo groupInfo2 = (GroupInfo) objArr[(i4 << 3) + i6];
                        if (groupInfo2.getNodeIndex() >= nodeIndex2 && !Intrinsics.areEqual((Object) groupInfo2, (Object) groupInfo) && (nodeIndex = groupInfo2.getNodeIndex() + nodeCount) >= 0) {
                            groupInfo2.setNodeIndex(nodeIndex);
                        }
                    }
                    j >>= 8;
                }
                if (i5 != 8) {
                    return true;
                }
            }
            if (i4 == length) {
                return true;
            }
            i4++;
        }
    }

    public final int slotPositionOf(KeyInfo keyInfo) {
        GroupInfo groupInfo = this.groupInfos.get(keyInfo.getLocation());
        if (groupInfo != null) {
            return groupInfo.getSlotIndex();
        }
        return -1;
    }

    public final int nodePositionOf(KeyInfo keyInfo) {
        GroupInfo groupInfo = this.groupInfos.get(keyInfo.getLocation());
        if (groupInfo != null) {
            return groupInfo.getNodeIndex();
        }
        return -1;
    }

    public final int updatedNodeCountOf(KeyInfo keyInfo) {
        GroupInfo groupInfo = this.groupInfos.get(keyInfo.getLocation());
        return groupInfo != null ? groupInfo.getNodeCount() : keyInfo.getNodes();
    }
}

...
public final class LayoutIntrinsics {
    ...
    public final float getMaxIntrinsicWidth() {
        ...
        float f = (float) (boringMetrics != null ? boringMetrics.width : -1);
        if (f < 0.0f) {
            CharSequence charSequence2 = this.charSequence;
            f = (float) Math.ceil((double) Layout.getDesiredWidth(charSequence2, 0, charSequence2.length(), this.textPaint));
        }
        if (LayoutIntrinsics_androidKt.shouldIncreaseMaxIntrinsic(f, this.charSequence, this.textPaint)) {
            f += 0.5f;
        }

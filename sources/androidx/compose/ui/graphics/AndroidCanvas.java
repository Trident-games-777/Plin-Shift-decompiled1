...
public final class AndroidCanvas implements Canvas {
    ...
    public void m4083concat58bKbWc(float[] fArr) {
        if (!MatrixKt.m4498isIdentity58bKbWc(fArr)) {
    ...
    public void m4082clipRectN_I0leg(float f, float f2, float f3, float f4, int i) {
        this.internalCanvas.clipRect(f, f2, f3, f4, m4091toRegionOp7u2Bmg(i));
    }

    /* renamed from: clipPath-mtrdD-E  reason: not valid java name */
    public void m4081clipPathmtrdDE(Path path, int i) {
        Canvas canvas = this.internalCanvas;
        if (path instanceof AndroidPath) {
            canvas.clipPath(((AndroidPath) path).getInternalPath(), m4091toRegionOp7u2Bmg(i));
            return;
        }
    ...
    public final Region.Op m4091toRegionOp7u2Bmg(int i) {
        if (ClipOp.m4223equalsimpl0(i, ClipOp.Companion.m4227getDifferencertfAjoo())) {
    ...
    public void m4087drawLineWko1d7g(long j, long j2, Paint paint) {
        this.internalCanvas.drawLine(Offset.m3987getXimpl(j), Offset.m3988getYimpl(j), Offset.m3987getXimpl(j2), Offset.m3988getYimpl(j2), paint.asFrameworkPaint());
    }
    ...
    public void m4084drawCircle9KIMszo(long j, float f, Paint paint) {
        this.internalCanvas.drawCircle(Offset.m3987getXimpl(j), Offset.m3988getYimpl(j), f, paint.asFrameworkPaint());
    }
    ...
    public void m4085drawImaged4ec7I(ImageBitmap imageBitmap, long j, Paint paint) {
        this.internalCanvas.drawBitmap(AndroidImageBitmap_androidKt.asAndroidBitmap(imageBitmap), Offset.m3987getXimpl(j), Offset.m3988getYimpl(j), paint.asFrameworkPaint());
    }

    /* renamed from: drawImageRect-HPBpro0  reason: not valid java name */
    public void m4086drawImageRectHPBpro0(ImageBitmap imageBitmap, long j, long j2, long j3, long j4, Paint paint) {
        ...
        rect.left = IntOffset.m7241getXimpl(j);
        rect.top = IntOffset.m7242getYimpl(j);
        rect.right = IntOffset.m7241getXimpl(j) + IntSize.m7283getWidthimpl(j2);
        rect.bottom = IntOffset.m7242getYimpl(j) + IntSize.m7282getHeightimpl(j2);
        Unit unit = Unit.INSTANCE;
        Rect rect2 = this.dstRect;
        Intrinsics.checkNotNull(rect2);
        rect2.left = IntOffset.m7241getXimpl(j3);
        rect2.top = IntOffset.m7242getYimpl(j3);
        rect2.right = IntOffset.m7241getXimpl(j3) + IntSize.m7283getWidthimpl(j4);
        rect2.bottom = IntOffset.m7242getYimpl(j3) + IntSize.m7282getHeightimpl(j4);
        Unit unit2 = Unit.INSTANCE;
        canvas.drawBitmap(asAndroidBitmap, rect, rect2, paint.asFrameworkPaint());
    }

    /* renamed from: drawPoints-O7TthRY  reason: not valid java name */
    public void m4088drawPointsO7TthRY(int i, List<Offset> list, Paint paint) {
        if (PointMode.m4564equalsimpl0(i, PointMode.Companion.m4568getLinesr_lszbg())) {
            drawLines(list, paint, 2);
        } else if (PointMode.m4564equalsimpl0(i, PointMode.Companion.m4570getPolygonr_lszbg())) {
            drawLines(list, paint, 1);
        } else if (PointMode.m4564equalsimpl0(i, PointMode.Companion.m4569getPointsr_lszbg())) {
    ...
    private final void drawLines(List<Offset> list, Paint paint, int i) {
        if (list.size() >= 2) {
            ...
            while (i2 < list.size() - 1) {
                long r0 = list.get(i2).m3997unboximpl();
                long r3 = list.get(i2 + 1).m3997unboximpl();
                long j = r3;
                long j2 = j;
                this.internalCanvas.drawLine(Offset.m3987getXimpl(r0), Offset.m3988getYimpl(r0), Offset.m3987getXimpl(j2), Offset.m3988getYimpl(j2), asFrameworkPaint);
                i2 += i;
            }
        }
    }
    ...
    public void m4089drawRawPointsO7TthRY(int i, float[] fArr, Paint paint) {
        if (fArr.length % 2 != 0) {
            throw new IllegalArgumentException("points must have an even number of values");
        } else if (PointMode.m4564equalsimpl0(i, PointMode.Companion.m4568getLinesr_lszbg())) {
    ...
    public void m4090drawVerticesTPEHhCM(Vertices vertices, int i, Paint paint) {
        this.internalCanvas.drawVertices(AndroidVertexMode_androidKt.m4134toAndroidVertexModeJOOmi9M(vertices.m4684getVertexModec2xauaI()), vertices.getPositions().length, vertices.getPositions(), 0, vertices.getTextureCoordinates(), 0, vertices.getColors(), 0, vertices.getIndices(), 0, vertices.getIndices().length, paint.asFrameworkPaint());
    }

    private final void drawPoints(List<Offset> list, Paint paint) {
        int size = list.size();
        for (int i = 0; i < size; i++) {
            long r2 = list.get(i).m3997unboximpl();
            this.internalCanvas.drawPoint(Offset.m3987getXimpl(r2), Offset.m3988getYimpl(r2), paint.asFrameworkPaint());
        }
    }
}

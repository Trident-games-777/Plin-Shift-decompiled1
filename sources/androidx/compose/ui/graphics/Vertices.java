...
public final class Vertices {
    ...
    public final int m4684getVertexModec2xauaI() {
    ...
    private final int[] encodeColorList(List<Color> list) {
        ...
        for (int i = 0; i < size; i++) {
            iArr[i] = ColorKt.m4293toArgb8_81llA(list.get(i).m4249unboximpl());
        }
        return iArr;
    }
    ...
    private final float[] encodePointList(List<Offset> list) {
        ...
        for (int i = 0; i < size; i++) {
            long r3 = list.get(i / 2).m3997unboximpl();
            if (i % 2 == 0) {
                f = Offset.m3987getXimpl(r3);
            } else {
                f = Offset.m3988getYimpl(r3);
            }
            fArr[i] = f;
        }
        return fArr;

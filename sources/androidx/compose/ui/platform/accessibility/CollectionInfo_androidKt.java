    ...
    private static final boolean calculateIfHorizontallyStacked(List<SemanticsNode> list) {
        ...
        if (list.size() == 0 || list.size() == 1) {
            ...
            while (i < lastIndex) {
                ...
                list2.add(Offset.m3976boximpl(OffsetKt.Offset(Math.abs(Offset.m3987getXimpl(semanticsNode4.getBoundsInRoot().m4017getCenterF1C5BW0()) - Offset.m3987getXimpl(semanticsNode3.getBoundsInRoot().m4017getCenterF1C5BW0())), Math.abs(Offset.m3988getYimpl(semanticsNode4.getBoundsInRoot().m4017getCenterF1C5BW0()) - Offset.m3988getYimpl(semanticsNode3.getBoundsInRoot().m4017getCenterF1C5BW0())))));
                semanticsNode = semanticsNode2;
            }
        }
        if (list2.size() == 1) {
            j = ((Offset) CollectionsKt.first(list2)).m3997unboximpl();
            ...
            if (1 <= lastIndex2) {
                int i2 = 1;
                while (true) {
                    first = Offset.m3976boximpl(Offset.m3992plusMKHz9U(((Offset) first).m3997unboximpl(), ((Offset) list2.get(i2)).m3997unboximpl()));
                    if (i2 == lastIndex2) {
                        break;
                    }
                    i2++;
            ...
            j = ((Offset) first).m3997unboximpl();
        } else {
            throw new UnsupportedOperationException("Empty collection can't be reduced.");
        }
        return Offset.m3988getYimpl(j) < Offset.m3987getXimpl(j);
    }
